...
def jwt_required(func):
    ...
    def jwt_required_wrapper(*args, **kwargs):
        ...
        try:
            ...
            return jsonify( {'error' : 'Token is invalid,please login again'}), 401
...
def upload():
    ...
    #视频文件容器
    video_client = blob_service_client.get_blob_client(container="<container.name>", blob=videoName)
    cover=files["cover"]
    coverName=cover.filename
    cover.save(tempFilePath+"/"+coverName)
    cover_path = os.path.join(tempFilePath, cover.filename)
    #图片文件容器
    cover_client = blob_service_client.get_blob_client(container="<container.name>", blob=coverName)
    try:
        block_list=[]
        ...
        #容器路径加文件名生成文件路径
        coverurl="<container.url>"+coverName
        videourl="<container.url>"+videoName
        info={
            "title":data["title"],#标题
        ...
        return make_response(jsonify( {'success':"Upload video success."}), 200)
    except:
...
def show_one_video(id):
    ...
    if video is not None:
        ...
        #检查user的收藏中是否有视频，check是为true视频已经被收藏
...
def add_subtitle(id):
    ...
    if video is not None:
        ...
        subtitle_client = blob_service_client.get_blob_client(container="<container.name>", blob=subtitleName)
        try:
