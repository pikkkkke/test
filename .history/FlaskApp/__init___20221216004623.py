...
def register():
    ...
    if not data.get("email"):
        return make_response(jsonify({"error": "Please pass the email"}), 400)
    if not data.get("username"):
        return make_response(jsonify({"error": "Please pass the username"}), 400)
    if not data.get("password"):
        return make_response(jsonify({"error": "Please pass the password"}), 400)
    email = data["email"]
