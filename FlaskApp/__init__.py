...
# 相关配置
video_container = "cloudprojectvideo"
cover_container = "cloudprojectpicture"
tvtt_container = "cloudprojectpicture"
...
@app.route('/cloud/api/v1/status', methods=['GET'])
@jwt_required
def status():
    ...
    if email != "":
        ...
        if user is not None:
            if bcrypt.checkpw(bytes(password, 'UTF-8'), user["password"]):
                ...
                ) + datetime.timedelta(minutes=30), 'userid': str(user['_id']), 'username': user['username']}, app.config['SECRET_KEY'], 'HS256')
...
def delete_video(id):
    ...
    if video is not None:
        # 替换文件url获取blob名字
        videoName = video["video"].replace(video_container+"/", "")
        video_client = blob_service_client.get_blob_client(
            container=video_container, blob=videoName)
        coverName = video["cover"].replace(cover_container+"/", "")
        cover_client = blob_service_client.get_blob_client(
...
@ app.route("/cloud/api/v1/video/<string:id>/collection", methods=["POST"])
@ jwt_required
def add_collection(id):
    ...
    if video is not None:
        ...
        users.update_one({"_id": ObjectId(user_data['userid'])}, {
                         "$push": {"collectVideo": id}})
        return make_response(jsonify({"message": "Add Collect Success"}), 201)
    else:
        return make_response(jsonify({"message": "Wrong video ID"}), 404)
...
@ app.route("/cloud/api/v1/video/<string:id>/collection", methods=["DELETE"])
@ jwt_required
def cancel_collection(id):
    ...
    if video is not None:
        ...
        videos.update_one({"_id": ObjectId(id)}, {
                          "$set": {"collect": collectNum-1}})
        users.update_one({"_id": ObjectId(user_data['userid'])}, {
                         "$pull": {"collectVideo": id}})
        return make_response(jsonify({"message": "Cancel Collect Success"}), 204)
    else:
        return make_response(jsonify({"message": "Wrong video ID"}), 404)
...
def show_all_collection():
    ...
    if user.get("collectVideo") is None or len(list(user.get("collectVideo"))) == 0:
        return make_response(jsonify({"message": "no collectvideos"}), 404)
