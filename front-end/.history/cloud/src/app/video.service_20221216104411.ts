...
export class VideoService {
  ...
  getVideos(pageInfo: pageInfo) {
    return this.http.get(`http://127.0.0.1:5000/api/v1.0/videos?pn=${pageInfo.page_num}&ps=${pageInfo.page_size}`);
  }
  ...
  getVideoByFilter(content: string, pageInfo: pageInfo) {
    // [{"filter": "views","gt": 1000,"lt": 100000000}]
    let formData = new FormData()
    formData.append("filters", `[{"filter": "title","contains": "${content}"}]`)
    return this.http.post(`http://127.0.0.1:5000/api/v1.0/videos?pn=${pageInfo.page_num}&ps=${pageInfo.page_size}`, formData);
  }
