...
export class InfoComponent implements OnInit, OnDestroy {
  ...
  thumb_up() {
    this.video.thumb_up(this.video_info._id).subscribe(val => {
      let m = (val as ret).message
      if (m == "liked") {
        this.video_info.likes += 1
      } else {
        this.video_info.likes -= 1
      }
    }, error => {
  ...
  thumb_down() {
    this.video.thumb_down(this.video_info._id).subscribe(val => {
      let m = (val as ret).message
      if (m == "disliked") {
        this.video_info.dislikes -= 1
      } else {
        this.video_info.dislikes += 1
      }
    }, error => {
